...
import { PatrontsDefaultError } from "../models/errors/patrontsdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PatrontsDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PatrontsDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PatrontsDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PatrontsDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
